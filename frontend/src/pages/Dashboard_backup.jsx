...
import { TrendingUp, ShoppingCart, DollarSign, Plus, BarChart3, Settings, Menu, Calendar, Search, Moon, Sun, LogOut, Home, Package, PieChart, RefreshCw, X, MessageCircle } from 'lucide-react';
...
export default function Dashboard({ user, onLogout, isDarkMode, setIsDarkMode }) {
  ...
  // Force re-render when currentPage changes
  useEffect(() => {
    // This ensures component re-renders when currentPage changes
  }, [currentPage]);
...
Generated by ProShop Business Dashboard
  ...
  return (
    <div style={{ 
      ...
      paddingBottom: '0' // Removed space for bottom nav
    }}>
      <style>{`
              ...
              <div style={{ position: 'relative' }}>
                ...
                {showUserMenu && (
                  <>
                    ...
                    >
                      <div style={{ padding: isMobile ? '8px 16px' : '12px 16px', borderBottom: `1px solid ${borderColor}` }}>
                        <p style={{ margin: 0, fontWeight: '600', color: textColor, fontSize: isMobile ? '12px' : '14px' }}>{user.username}</p>
                        <p style={{ margin: '2px 0 0 0', fontSize: isMobile ? '10px' : '12px', color: secondaryText }}>{user.email || 'No email'}</p>
                      </div>
          ...
          {activeNav === 'dashboard' && (
            <>
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ 
                        ...
                        <card.icon color={card.color} size={isMobile ? 16 : 28} />
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {showMobileSupport && (
                <MobileSupport 
                  isDarkMode={isDarkMode} 
                  onClose={() => setShowMobileSupport(false)} 
                />
              )}
            </>
          )}
      ...
      <>
      {/* Bottom Navigation (Mobile Only) - DISABLED */}
      {false && isMobile && (
        <div style={{
          position: 'fixed',
          bottom: 0,
